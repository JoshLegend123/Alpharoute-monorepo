 ...
 */
const PACKAGE_WHITELIST: Record<'mainnet' | 'testnet', Set<string>> = {
  mainnet: new Set([
...
/**
 * Flowchart step structure for frontend rendering.
 */
...
export class TheGuardian {
   ...
   */
  public async validateTransaction(ptb: any): Promise<ValidationResult> {
    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      if (command.kind === 'MoveCall') {
        ...
        if (!this.whitelist.has(packageId)) {
          result.safe = false;
          result.reasons.push(
            `SECURITY WARNING: Unauthorized Move package ID detected: "${packageId}". ` +
   ...
   */
  private resolveArgument(arg: any, inputs: any[]): any {
  ...
  private validateSlippage(
    command: any,
    inputs: any[]
  ): { isSafe: boolean; isWarning: boolean; reason: string } {
   ...
   */
  private generateFlowchartStep(step: number, command: any, inputs: any[]): FlowchartStep {
    ...
    switch (command.kind) {
      case 'MoveCall': {
        ...
        const amountArg = command.arguments?.find((arg: any) => {
